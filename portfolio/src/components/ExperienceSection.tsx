          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              <div key={index} className={`relative flex flex-col md:flex-row gap-8 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                {/* Timeline Dot */}
                <div className="absolute left-0 md:left-1/2 top-8 w-4 h-4 bg-primary border-2 border-background rounded-full transform -translate-x-1/2 hidden md:block z-10"></div>

                {/* Content */}
                <div className="w-full md:w-1/2">
                  <Card hoverEffect className={`group border-border ${index % 2 === 0 ? 'md:mr-12' : 'md:ml-12'}`}>
                          ...
                          <ul className="space-y-2">
                            {exp.responsibilities[language].map((resp, idx) => (
                              <li key={idx} className="text-sm text-muted-foreground flex items-start gap-2">
                                <span className="text-primary mt-1.5 min-w-[4px] h-[4px] rounded-full bg-primary"></span>
                                <span>{resp}</span>
                              </li>
                            ))}
                          </ul>
